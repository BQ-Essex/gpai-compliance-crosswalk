...
**What a green board does not mean** is the part worth reading, and `check.py` says it on every clean run. Every one of the seventeen errors in the corrections log passed every check that existed when it was made. These establish that citations resolve, that quotations are accurate, that verdicts stay in register and that absences are bounded. They cannot establish that a provision is the right one for the proposition, that an inference from two recitals is sound, or that a reading of a Commission opinion is the better of two available readings—which is where this analysis has actually gone wrong. Green is a floor, not a verdict.
...
Soundness is not a decidable property and no tool here will ever say an argument is good. But the reason this analysis’s seventeen errors survived was not that a script could not adjudicate them. It was that they sat in prose, where nobody was reading them as claims.

`data/inferences.yaml` gives reasoning the treatment this repository already gives evidence and legal conclusions: its own cell, so the failure can be looked at. Nineteen argumentative steps, each stated so it could be denied, each resting on premises that must resolve to the register, each naming **what would defeat it**, and each ranked `strong`, `moderate` or `contestable` by its own author.
