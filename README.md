...
2. **Hold the record before characterising it.** Every document the analysis will cite goes in `_sources/` with its SHA-256 in `data/sources.yaml`. **[`docs/holding-a-source.md`](docs/holding-a-source.md) is the two-minute procedure**, and `tools/hold-source.py` does the half of it a tool can. Twenty-one of this project’s thirty errors were a source characterised without being opened, and `tools/validate.py` now refuses a fact whose source is not held.
