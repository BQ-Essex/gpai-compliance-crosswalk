...
Thirteen errors have been found in this analysis. Four came from adversarial review of the finished draft; the fifth from a checker written afterwards; the sixth from reading a cross-referenced directive instead of a summary of it; the seventh from a pre-print’s version history rather than its title page; the eighth from reading the amending act rather than inferring from its footprints; the ninth from reading a document rather than its landing page; the tenth from reading Article 52 rather than assuming its shape; the eleventh from counting this table instead of quoting a sentence about it; the twelfth from searching a document this project had held since the first day; and the thirteenth from checking quotations in the register, which had never been checked. **Nine of the thirteen are the same failure: characterising a source without opening it**—and the twelfth is its close cousin, opening a source for one question and never asking it a second. The tenth, part of the eleventh and the whole of the thirteenth are the ones the tooling caught before any human read the sentence. All are recorded rather than silently fixed.
