...
In total, twenty-eight errors have been found in this analysis. Four came from adversarial review of the finished draft; the fifth from a checker written afterwards; the sixth from reading a cross-referenced directive instead of a summary of it; the seventh from a pre-print’s version history rather than its title page; the eighth from reading the amending act rather than inferring from its footprints; the ninth from reading a document rather than its landing page; the tenth from reading Article 52 rather than assuming its shape; the eleventh from counting this table instead of quoting a sentence about it; the twelfth from searching a document this project had held since the first day; the
thirteenth from checking quotations in the register, which had never been checked; and the fourteenth from taking a reviewer’s word for what a provision says; and the fifteenth from describing that reviewer as a lawyer; and the sixteenth from marking its citations unverified instead of verifying them; and the seventeenth from a comparative appendix that cited two foreign statutes and registered neither; and the eighteenth from holding the incident record as files for the first time; and the nineteenth from an appendix contradicting this project’s own register; and the twentieth to twenty-fourth from an external adversarial audit on the final day, which opened the held sources against the report’s sentences and found five places where the sentence said more than the source; and the twenty-fifth from writing a re-verifier for the statutory register and running it once; and the twenty-sixth from making the method’s first rule a checker and running it on the method’s own first run; and the twenty-seventh from reading the finished report end to end on the last morning; and the twenty-eighth from checking whether the register’s oldest entries had kept up with its own conventions. **Twenty of the twenty-eight are the same failure: characterising a source without opening it**—and the twelfth is its close cousin, opening a source for one question and never asking it a second. The tenth, part of the eleventh, and the whole of the thirteenth and fourteenth are the ones the tooling caught before any human read the sentence. All are recorded rather than silently fixed.
...
| **The scope verdict stated one open condition where the route it relied on needs three, resting on a premise the T1 source denies** | §1.8 and the report’s §4.1 said the obligations attach “provided the model is one with systemic risk”, and `inf-product-oriented` rested on “a model it intends to ship”. The provider’s own account, quoted at §1 of the report, says the model was one of several “research models that were not intended for public release” and that its weights were quarantined; the Q&A’s development-phase reading is expressly for models “intended for, but prior to, the placing on the market”; and the only system the model is recorded as running in is the evaluation harness, which is testing before putting into service, the very thing Article 2(8) excludes. The route needs a **placement event** (integration into an own system put into service, an intention to place, or a shared large pre-training run with a placed model, Guidelines ¶22) and a **Union nexus** (Article 2(1)(a), Article 3(11), Guidelines ¶51), neither of which the record supplies, and the inference register had no node for either. Both are now inferences (`inf-placement-event`, `inf-union-nexus`), both marked contestable, both asked by the instrument (Requests 2b and 4a). The same audit found the Milch-Kontor question answered with a shrug: Recital 97 has two limbs, only the put-into-service limb is available to a never-released model, and that is the limb the enacting text’s “supply” wording does not obviously cover. §1.8 and the report now say so. **Pistillo’s subject is internal *deployment***; his ¶¶55–57 place post-training inside the pre-deployment activity Article 2(8) protects, and the report had borrowed his conclusion for a fact pattern he would exclude. |
| **“Every reporting period in the Code runs from awareness”, four times, including the abstract** | Measure 9.3 runs each **initial-report** period from awareness. Intermediate reports follow “at least every four weeks after the initial report”; the final report is due “not later than 60 days after the serious incident has been resolved”. The Measure had been read for its initial periods and characterised for all of them. Now “every initial-report period”. Found with it: the report called the bifurcation the omission’s “cause” and said the two forms differ “for no reason this analysis can find”, while `docs/appendix-forms.md` had recorded since 13 September that fields 1 to 9 of the template transcribe Measure 9.2 item for item. The reason was in the repository. The omission is the Code’s, inherited by the form; the report now says so and drops the causal claim. |
| **Two absences upgraded to refusals, by the method that exists to keep them apart** | Appendix G said the Commission’s spokesperson “declined to give the filing date, the provision relied on, or whether the episode qualifies as a serious incident”. The held press report says he “declined to say when the company submitted it or disclose its contents”, and that the Commission “has not said” whether the behaviour falls within Article 55. One refusal, two absences. The report’s own distinction—a refusal is evidence, an absence is the fallback—misapplied in the paragraph that states it. |
| **An exception recorded as a fix, and a quotation attributed to the wrong account** | `data/unverifiable-quotations.yaml` said of “intentionally disabled for the cyber evaluations” that “the sentence now quotes the sources instead”. The report’s §4.2 still carried the phrase in quotation marks attributed to the investigators, who say the classifiers “were intentionally off for the cyber evaluations”; the exception, filed as own-words, let it through `quotecheck.py` at 171 of 171. The same paragraph attributed “also reproduced an exploit” to the investigators; it is the provider’s sentence. `quotecheck.py` matches a quotation against any held document by design, so “quotations match the documents they are attributed to” claimed more than the tool checks. Both fixed in the sentence; the exception now says what it protects. |
| **A precautionary remediation presented as the “principal disclosed consequence”** | §4.4 said the principal disclosed consequence was one core cluster “wiped and rebuilt from scratch”. The affected party’s timeline says it did so “as an extra precaution”. That is a remediation choice rather than damage the agents caused, which is the provider-favourable reading of limb (d) and the facts-versus-characterisation line this project polices everywhere else. Found with it: `inf-no-101a-exposure` said July 2026 conduct carries **no** Article 101(1)(a) exposure, which Article 113 does not establish for an omission that continued past 2 August 2026; withdrawn to a stated non-position. And the report’s §4.3 presented the designation power as a way past the scope question, when Article 52(1)’s closing sentence and Annex XIII both operate on “a general-purpose AI model”. |
| **Four register extracts not verbatim, found the hour a re-verifier existed** | `tools/reverify.py` compares every entry in `data/provisions.yaml` against the text layer of the consolidated Regulation and the OJ texts on their alphanumeric residue. Its first run found Article 53(1), point (a) with its “(a)” dropped; Article 3, point (57) with a comma added before “and it does not qualify”; Article 75(1a) with the Act’s own stray comma after “(9)” removed; and the CER Annex, sector 8 abridged—three repeated “of Directive (EU) 2022/2555” and a footnoted citation of Regulation (EU) No 910/2014 silently dropped rather than elided. None changed a meaning; all four were in a register whose header claims verbatim text, and one had a `verified` date beside it. All 55 checkable entries now match, and each carries the date and file names of the check that says so. |
| **Three facts resting on sources nobody held, and five sources said to establish things nobody had opened, undetected because no checker looked** | The second run broke the method’s first rule twice in its first hour, and asking whether a checker could have stopped it produced one, `tools/validate.py`, which now refuses a fact whose source has neither a SHA-256 nor a Wayback capture, and refuses an `establishes:` line on any such source. Run on the first run it found three: the claim that the UK AI Security Institute found all five frontier models tested attempted to cheat rests on a Cloud Security Alliance article never held; the claim that the provider is a full Code signatory rests on a third-party registry page never held; and five sources—the same two, California SB 53, the EU SEND guidance and the transparency-code opinion—carried `establishes:` lines written from pages never fixed. The AISI and signatory claims appear in the report’s §3 and §5.2. Twenty of twenty-six errors are now the same failure. |
| **The report’s appendices ran A B C D E G F H** | Appendix G was inserted ahead of Appendix F when it was created and nothing looked at the sequence. Both existed, both were cited, and `dangling_appendices` in `tools/housestyle.py` checked only that a cited appendix was defined—not that the defined ones were in order. Found on the final read-through, which is the one check with no tool behind it. The order is fixed and the ordering is now checked, with a negative control run before it was trusted. Presentation rather than analysis, and a judge notices it in the same second as a dangling reference |
| **A hash with nothing naming the file it hashed, on the most load-bearing source in the timing analysis** | `ec-cop-adequacy-opinion` and `ec-ai-board-adequacy` each carried a `sha256:` and no `document:`. A reader had to guess which file in `_sources/` the number belonged to, and guessing is what the register exists to remove. Both hashes proved **correct** on checking, so this is a documentation defect and not an integrity one—and it is worth recording for the class rather than the instance. **When a convention is introduced, nothing sweeps the entries that predate it.** These two were registered before `document:` and `held_from:` existed, and every check written since has tested the entries that have those fields. `tools/validate.py` now refuses a sha256 that names no file, with a negative control run before it was trusted |
...
Scientific research and development is excluded outright, on a **sole purpose** test, by Article 2(6) and this recital together. Product-oriented research is not excluded at all: the Regulation simply does not reach it *before* placement or putting into service. A cyber-capability evaluation the provider says it runs “to determine the appropriate safeguards for deployment” is product-oriented on any reading, so this half of the recital postpones rather than exempts—and a postponement only matters for a model that is later placed, which is the question §1.8 now puts first. An earlier version of this paragraph said “a model it intends to ship”; the provider’s account says the opposite, and the twentieth entry in the log records it.
...
**The obligations attach**, on the Commission’s reading, if three facts hold. The route runs: model integrated into the provider’s own AI system put into service, in the Union → deemed placed on the market (Recital 97) → internal-use exception unavailable because the model carries systemic risk → Chapter V applies, including the development-phase duties the Q&A names.

**The live questions are factual, and there are three, not one.** (1) *Systemic risk*: does the model exceed the 10²⁵ FLOP threshold at Article 51(2)? (2) *A placement event*: was the model ever integrated into an own system put into service; or was it “intended for” placement, which is the Q&A’s own condition; or does it share a large pre-training run with a model that was placed, in which case paragraph 22 of the Guidelines makes it that model? The provider’s account says it was one of several “research models that were not intended for public release” and that its weights were quarantined; the only system it is recorded as running in is the evaluation harness, and testing before putting into service is what Article 2(8) excludes. So the record supplies none of the three routes and denies one. (3) *The Union*: Article 2(1)(a) reaches placing on the market “in the Union”, Article 3(11) “own use in the Union”, and the Guidelines’ internal-use example at paragraph 51 requires processes essential to third-party provision “or that affect the rights of natural persons in the Union”. Nothing in the record addresses it. If all three hold, Article 52(1)’s two-week notification duty was engaged during development, and Articles 53 and 55 apply. The instrument asks all three (Requests 2, 2b, 4a).

**Four qualifications belong in any honest statement of this.**
...
4. **Deutsches Milch-Kontor bites unevenly, and on the limb this model would need.** Recital 97 deems placement where an own model is integrated into an own system “made available on the market or put into service”. Where the system is made available on the market, the embedded model is supplied for use on the Union market in the course of a commercial activity, which is what Article 3(9) and (10) say placing means; the recital is arguably applying the definition, not contradicting it. Where the system is merely put into service, nothing is supplied to anyone, and putting into service is the trigger Article 2(1)(a) withholds from models. A never-released model can only ever use the second limb. The Commission does not rest that limb on the recital alone—paragraph 51 of the Guidelines lists essential internal use among its examples of placing on the market, read with the Blue Guide and Articles 3(9) and (10), which is the extensive reading of ‘making available’ Pistillo runs as his second pathway—so a challenge is really against that reading of ‘supply’. Whether own use can be a supply is the question, and the enacting text does not settle it. Stated, not resolved.
