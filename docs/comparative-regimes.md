...
| Non-compliance independently actionable | Yes — §22757.15, up to $1m per violation | Yes — via Art. 55(2) and Art. 101(1)(a) |
...
California can fine a developer for departing from its own published framework. The EU can **compel production of the underlying documents first**, at a penalty ceiling three orders of magnitude higher, and can fine separately under Article 101(1)(b) for failing to comply with that request. For an episode where the central difficulty is that nobody outside the provider can see the relevant documents, the information power matters more than the penalty.

## 5. What this comparison establishes, and what it does not

**Establishes**, on the face of the three texts:
1. The EU threshold (10²⁵) reaches models the US state regimes (10²⁶ plus a revenue gate) do not — including, possibly, the model at the centre of this incident.
